"""ADAPT-QAOA with a counterdiabatic-inspired operator pool.

Pool: {Y_i, Y_i X_{i+1}, X_i Y_{i+1}}.
At each ADAPT step, gradient ||<psi|[H, A]|psi>|| picks the next operator;
all parameters are re-optimized. Terminates when |grad| < tol or max_layers.
"""
...
def _build_circuit(N: int, layers: list, params_sym):
    from qiskit import QuantumCircuit
    from qiskit.circuit.library import PauliEvolutionGate
    qc = QuantumCircuit(N)
    qc.h(range(N))
    for k, (kind, i, j, op) in enumerate(layers):
        qc.append(PauliEvolutionGate(op, time=params_sym[k]), range(N))
    return qc
...
def solve(
    ...
    def build_circuit_with(layers_list, thetas):
        qc = QuantumCircuit(N)
        qc.h(range(N))
    ...
    for _ in range(max_layers):
        if not layers:
            base_state_circ = QuantumCircuit(N); base_state_circ.h(range(N))
        else:
            base_state_circ = build_circuit_with(layers, theta)

        commutators = []
        for (kind, i, j, A) in pool:
            # i*[H,A] is Hermitian when H, A are Hermitian; gradient = <psi| i[H,A] |psi>
            comm = (1j * (H @ A - A @ H)).simplify()
            commutators.append(comm)

        circ_t = transpile(base_state_circ, basis_gates=["sx", "rz", "cx"], optimization_level=1)
        observables = [c.apply_layout(circ_t.layout) if circ_t.layout is not None else c for c in commutators]
        pub = (circ_t, observables)
        ...
        def cost_fn(p):
            ...
            r = estimator.run([pub2]).result()
            e = float(r[0].data.evs[0])
